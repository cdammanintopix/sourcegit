...
namespace SourceGit.Views
{
    ...
    public partial class Histories : UserControl
    {
            ...
            if (!repo.IsBare)
            {
                ...
                if (commit.IsMerged && commit.Parents.Count > 0)
                {
                    ...
                    if (!isHead)
                    {
                        ...
                        fixupWithStaged.Header = App.Text("CommitCM.InteractiveRebase.FixupWithStaged");
                        ...
                        fixupWithStaged.Click += async (_, e) =>
                        {
                            ...
                            if (succ)
                            {
                                ...
                                await this.ShowDialogAsync(new ViewModels.InteractiveRebase(repo, parent, new ViewModels.InteractiveRebasePrefill(commit.SHA, Models.InteractiveRebaseAction.Fixup)));
                            }
                            e.Handled = true;
                        };
        ...
        private void FillGitLabMenu(ContextMenu menu, ViewModels.Repository repo, Models.Commit commit)
        {
            ...
            gitlab.Icon = this.CreateMenuIcon("Icons.GitLab");
